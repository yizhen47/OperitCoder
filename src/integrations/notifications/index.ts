...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		// Add Operit Coder logo
		const extensionUri = vscode.extensions.getExtension(`kilocode.operit-coder`)!.extensionUri
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Operit Coder").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "Operit Coder", message } = options
