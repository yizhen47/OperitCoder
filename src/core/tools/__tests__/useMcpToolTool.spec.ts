...
describe("useMcpToolTool", () => {
	...
	describe("successful execution", () => {
		it("should retry Playwright with edge then chromium when chrome is missing", async () => {
			const block: ToolUse = {
				type: "tool_use",
				name: "use_mcp_tool",
				params: {
					...
					arguments: '{"channel":"chrome"}',
				},
				partial: false,
			}

			const chromeMissingError = new Error(
				"browserType.launchPersistentContext: Chromium distribution 'chrome' is not found",
			)
			const edgeMissingError = new Error(
				"browserType.launchPersistentContext: Chromium distribution 'msedge' is not found",
			)

			const callToolMock = vi
				.fn()
				.mockRejectedValueOnce(chromeMissingError)
				.mockRejectedValueOnce(edgeMissingError)
				.mockResolvedValueOnce({
					content: [{ type: "text", text: "Opened with chromium" }],
					isError: false,
				})
			...
			mockProviderRef.deref.mockReturnValue({
				getMcpHub: () => ({
					getAllServers: vi.fn().mockReturnValue([
						{ name: "playwright", tools: [{ name: "browser_open", description: "Open browser" }] },
					]),
			...
			expect(callToolMock.mock.calls[1][2]).toMatchObject({ channel: "msedge" })
			expect(callToolMock.mock.calls[2][2]).toMatchObject({ channel: "chromium" })
			expect(mockTask.say).toHaveBeenCalledWith("mcp_server_response", "Opened with chromium")
		})

		it("should auto-approve Playwright MCP tools", async () => {
			const block: ToolUse = {
				type: "tool_use",
				name: "use_mcp_tool",
				params: {
			...
			const mockToolResult = {
				content: [{ type: "text", text: "Opened browser" }],
				isError: false,
			}
			...
			expect(mockTask.say).toHaveBeenCalledWith("mcp_server_response", "Opened browser")
		})
